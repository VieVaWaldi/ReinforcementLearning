...
class ExperienceBuffer:
    ...
    def sample(self, batch_size):
        ...
        return np.array(states), np.array(actions), np.array(
            rewards, dtype=np.float32), np.array(dones, dtype=np.uint8), \
...
if __name__ == "__main__":
    ...
    net = DQN(env.get_observation_size(), env.get_action_size()).to(device)
    tgt_net = DQN(env.get_observation_size(), env.get_action_size()).to(device)
    ...
    while True:
        ...
        if reward is not None:
            ...
            if best_mean_reward is None or best_mean_reward < mean_reward:
                torch.save(net.state_dict(), NAME + "-best.dat")
